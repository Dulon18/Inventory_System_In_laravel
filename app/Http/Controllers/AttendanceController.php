...
class AttendanceController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        {
            $date=Attendance::where('edit_date',$id)->first();
            $data=Attendance::join('employees','attendances.emp_id','employees.id')
            ->select('employees.name','employees.image','attendances.*')
            ->where('edit_date',$id)->get();
            return view('pages.attendance.edit',compact('data','date'));
        }
